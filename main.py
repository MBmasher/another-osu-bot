...
async def on_message(message):
    ...
    if message.content.startswith('~recent') or message.content.startswith('~rs') or message.content.startswith('~r') and not message.content.startswith('~rb') and not message.content.startswith('~roll'):
    ...
    if message.content.startswith('~compare') or message.content.startswith('~c'):
        try:
            ...
            if number > 99:
                ...
                play_list, title_s, link_s, diff_s, user_info_s, user_link, user_pfp, b_id = recent.return_recent(author, 2,
                                                                                                                  number,
                                                                                                                  last_beatmap)
                if play_list != 5:
                    if len(play_list[0]) <= 1:
                        await client.send_message(message.channel, "{} has no plays on this beatmap!".format(author))
                    elif link_s == 0:
                        play = "play"
                        if len(play_list) > 1:
                            play += "s"
                        await client.send_message(message.channel, "{} has no more than {} {} on this beatmap!".format(author, len(play_list), play))
                    else:
                        emb = discord.Embed(title=title_s, description=diff_s, url=link_s)
                        emb.set_author(name=user_info_s, url=user_link, icon_url=user_pfp)
                        await client.send_message(message.channel, embed=emb)
    ...
    if message.content.startswith('~roll'):
        try:
            ...
            await client.send_message(message.channel, "<@{}> rolled a {} out of {}.".format(user_id, random.randint(1, number), number))
        except Exception as e:
            await client.send_message(message.channel, "<@203322898079809537> you fucked up dumbass\n{}".format(e))
    ...
    if (message.content.startswith('!r') or message.content.startswith('!rb') or message.content.startswith('!recent')
            or message.content.startswith('!top') or message.content.startswith('!best') or message.content.startswith('!rs') or
        ...
        await client.send_message(message.channel, "I'm online, use me!! (I'm definitely cooler than BoatBot or owo)")
